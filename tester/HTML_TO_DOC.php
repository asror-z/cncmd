...
EOH;
    ...
    public function createDoc_1($html){
        @header("Cache-Control: ");// leave blank to avoid IE errors
        @header("Expires: 0");// leave blank to avoid IE errors
        @header("Pragma: no-cache");// leave blank to avoid IE errors
        @header("Content-type: application/octet-stream");
     ...
     */
    public function write_file($file, $content, $mode = 'wb'){
        $fp = @fopen($file, $mode);
        if(!is_resource($fp)){
            return false;
        }
